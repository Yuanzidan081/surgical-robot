...
for i=1:len
%base=[i,0,0];
[e_f,c_f,l_f,h_f,f_f]=Get_Disk_Info(n,theta_f,phi_f(i),1.38,1.62,[0,0,0]);
x_test(i,:)=e_f(1);
y_test(i,:)=e_f(2);
z_test(i,:)=e_f(3);
eF(i,:)=e_f;
...
for i=1:len
    ...
    theta_i(i,:)=2*atan((sqrt((y_test(i))^2+(x_test(i))^2))/(z_test(i)))/(n-1);
end

for i=1:len
%base=[i,0,0];
[e_i,c_i,l_i,h_i,f_i]=Get_Disk_Info(n,theta_i(i),phi_i(i),1.38,1.62,[0,0,0]);
eI(i,:)=e_i;
%绘制Backbone
...
ylabel('y/mm');
zlabel('z/mm');
legend([h1,h2],{'正运动学绘制','逆运动学绘制'},'Location',[0.704064621471224 0.850793648291299 0.219642861521974 0.0841836759717404]);
